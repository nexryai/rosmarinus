...
const rules = {
    input: css({
        ...
        "&:focus": { borderColor: "var(--accent-hover)", background: "var(--panel)" },
    }),
};

export function RemoteUserSearch({ actorID, csrf, onOpenProfile }: { actorID: string; csrf: string; onOpenProfile: (actorID: string) => void }) {
        ...
        <>
            ...
            >
                ...
                <label htmlFor="remote-profile-target" style={styles.label}>
                    ...
                    <input autoCapitalize="none" autoComplete="off" className={rules.input} disabled={loading} id="remote-profile-target" onChange={(event) => setTarget(event.target.value)} placeholder="@alice@example.com" spellCheck={false} style={styles.input} value={target} />
                </label>
                <span style={styles.hint}>例: @alice@example.com または https://example.com/users/alice</span>
