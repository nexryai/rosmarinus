...
export function QuotedNoteCard({ onOpen, onOpenProfile, quote }: { onOpen?: (noteID: string) => void; onOpenProfile?: (actorID: string) => void; quote: Quote }) {
    ...
    const content: ReactNode = (
        <>
            ...
            {quote.content_warning ? <span style={styles.warning}>閲覧注意: {quote.content_warning}（詳細を開いて表示）</span> : <Mfm emojis={quote.emojis} mentions={quote.mentions} nyaize={author?.is_cat} onOpenProfile={onOpenProfile} style={styles.text} text={quote.text || "本文のないノート"} />}
        </>
    );
